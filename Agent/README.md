...
│  backend/  — NAPI 桥接封装                │
├──────────────────────────────────────────┤
│   独立 native builder (libcodexhost-builder/) │
...
| `src/main/libs/x86_64/` | 预编译 native 模块放置目录。`libcodexhost.so` 与其运行时依赖会在构建前由独立 builder 产出并安装到这里，随后由 HAP 重打包脚本注入最终安装包 |
| `src/main/types/libcodexhost/` | NAPI 类型声明。`index.d.ts` 声明 `NativeCodexHostStatus`、`NativeCodexProviderConfig` 等接口，供 ArkTS 侧类型检查 |
...
│   │   ├── libs/x86_64/               # 预编译 libcodexhost.so 与依赖
...
- 独立 builder：`..\libcodexhost-builder\`
- NAPI 桥接：`..\libcodexhost-builder\native\bridge\napi_init.cpp`
- 类型定义：`entry/src/main/types/libcodexhost/`
