...
const ALLOW_ORIGIN_ENV_VAR: &str = "CODEX_APP_SERVER_ALLOW_ORIGIN_HEADER";
...
async fn reject_requests_with_origin_header(
    ...
    if request.headers().contains_key(ORIGIN) && !allow_origin_header_override() {
        warn!(
...
fn allow_origin_header_override() -> bool {
    matches!(
        std::env::var(ALLOW_ORIGIN_ENV_VAR),
        Ok(value) if matches!(
